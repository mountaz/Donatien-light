...
public class MinimizerBarnesHut {
	...
	public void minimizeEnergy(final Map<Node,double[]> positions, int iterations) {
		startMinimizeEnergy(positions);
	 ...
	 */
	public void startMinimizeEnergy(final Map<Node,double[]> positions) {
		...
		computeBaryCenter();
		octTree = buildOctTree(); // for efficient repulsion computation
...
//		System.out.println("initial energy " + energySum);
		...
		// minimize energy
		oldPos = new double[nrDims];
		bestDir = new double[nrDims];
	...
	int step;
	OctTree octTree;
	double finalAttrExponent, finalRepuExponent;
	int nrIterations = 20;
	double energySum = 0;
	double[] oldPos = null, bestDir = null;
	double previousEnergySum;
	
	public boolean minimizeEnergyStep() {
		computeBaryCenter();
...
//			  + "   repulsion " + repuExponent);
		...
		step++;
		return (step > 10 && previousEnergySum == energySum);
	}
	
	public void endMinimizeEnergy() {
		printStatistics(octTree);
